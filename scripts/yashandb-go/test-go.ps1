...
Write-Host "YashanDB Go 驱动测试" -ForegroundColor Cyan
Write-Host "=========================================" -ForegroundColor Cyan
Write-Host ""
...
function Test-GoModule {
    Write-Status "测试 2: 检查 yasdb-go 模块是否已安装..."
...
function Test-CDriver {
    ...
    if ($yascliPath) {
        ...
        Write-ErrorMsg "未找到 C 驱动库 - Go 驱动需要此依赖"
        return $false
    }
}
...
if (Test-GoModule) { $passed++ } else { $failed++ }
Write-Host ""
